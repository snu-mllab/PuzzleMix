...
parser.add_argument('--padding', default=4, type=int,
                    help='padding for transform_train')

parser.add_argument('--method', default='puzzle', type=str,
                    help='mixup type', choices=('vanilla', 'cut', 'puzzle'))
# For Puzzle Mix
...
def main():
    ...
    if args.dataset.startswith('cifar'):
        ...
        if args.dataset == 'cifar100':
            ...
            val_loader = torch.utils.data.DataLoader(
                ...
                batch_size=args.batch_size//4, shuffle=True, num_workers=args.workers, pin_memory=True)
            numberofclass = 100
    ...
    if os.path.isfile(pretrained):
        ...
        print("=> loaded checkpoint '{}'(epoch: {}, best err1: {}%)".format(pretrained, cur_epoch, checkpoint['best_err1']))
    else:
        cur_epoch = 0
    ...
    for epoch in range(cur_epoch, args.epochs):
        ...
        # train for one epoch
        train_loss = train(train_loader, model, criterion, criterion_batch, optimizer, epoch, mean_torch, std_torch, mp)
...
def train(train_loader, model, criterion, criterion_batch, optimizer, epoch, mean, std, mp=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.beta > 0 and r < args.mixup_prob and args.method == 'cut':
            ...
            with torch.no_grad():
                input, lam = mixup_graph(input, unary, rand_index, block_num=block_num,  
                                         alpha=alpha, beta=args.p_beta, gamma=args.p_gamma, eta=args.p_eta,
                                         neigh_size=args.neigh_size, n_labels=args.n_labels,
                                         mean=mean, std=std, transport=args.transport, t_eps=args.t_eps, dataset=args.dataset, mp=mp)
            # calculate loss
            output = model(input)
            loss = lam * criterion_batch(output, target) + (1 - lam) * criterion_batch(output, target[rand_index])

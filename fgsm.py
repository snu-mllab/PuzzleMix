...
parser.add_argument('--train', type=str, default='vanilla', choices=['vanilla', 'mixup', 'mixup_hidden','cutout', 'mixup+clean', 'graph', 'clean', 'clean_graph', 'clean_mixup'])
...
def train(train_loader, model, optimizer, epoch, args, log, mean=None, std=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        input_fgsm = input*std + mean
        input_fgsm = input_fgsm + (args.eps/255.) * input_var.grad.sign()
        input_fgsm = torch.clamp(input_fgsm, 0, 1)
        input_fgsm = (input_fgsm-mean)/std

        model.train()

        if args.train == 'vanilla':
            output = model(input_fgsm)
            loss = criterion(output, target)

        elif args.train == 'mixup':
            permuted_idx = np.random.permutation(input.size(0))
            lam = np.random.beta(1, 1)

            input_fgsm = lam*input_fgsm + (1-lam)*input_fgsm[permuted_idx]
            output = model(input_fgsm)
            loss = lam*criterion(output, target) + (1-lam)*criterion(output, target[permuted_idx])

        elif args.train == 'mixup+clean':
            permuted_idx = np.random.permutation(input.size(0))
            lam = np.random.beta(1, 1)
            lam = max(lam, 1-lam)

            input = lam*input + (1-lam)*input_fgsm[permuted_idx]
            output = model(input)
            loss = lam*criterion(output, target) + (1-lam)*criterion(output, target[permuted_idx])
            
        elif args.train == 'graph':
            permuted_idx = np.random.permutation(input.size(0))
            lam = np.random.beta(1, 1)
            lam = max(lam, 1-lam)

            unary = torch.sqrt(torch.mean(input_var.grad**2, dim=1))
            input, lam = mixup_graph(input, unary, permuted_idx, alpha=lam, mean=mean, std=std)
            input = (2*input + input_fgsm) / 3
            output = model(input)
            lam = (2*lam+1) / 3
        ...
        elif args.train == 'clean':
            output = model(input)
            loss = criterion(output, target)
        
        elif args.train == 'clean_mixup':
            permuted_idx = np.random.permutation(input.size(0))
            lam = np.random.beta(1, 1)
            lam = max(lam, 1-lam)
